...
describe('earth#validateWearable', function () {
  it('ok', function () {
    const result = earth.validateWearable(human, earth, steelArmor, lightSword);
    assert.equal(result, null);
  });
  it('ng', function () {
    const result = earth.validateWearable(fairy, earth, steelArmor, lightSword);
    if (isNotWearableErorr(result)) {
      assert.equal(result.acquirement.name, 'earth');
      assert.equal(result.cause.name, 'fairy');
      assert.equal(result.message, 'このキャラクターの設定ではearthを装備できません');
    } else {
